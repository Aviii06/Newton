...
Intersection* intersect_ray_triangle(Ray* ray, const glm::vec3& V0, const glm::vec3& V1, const glm::vec3& V2)
{
	...
	if (fabs(b) < ERROR_TOLERANCE)
	{ // ray is parallel to triangle plane
		if (a == 0) // case 1: ray lies in triangle plane
			return NULL;
		else
			return NULL; // case 2: ray disjoint from plane
	}
	...
	for (int i = 0; i < verticesVector.size(); i++)
	{
		for (auto&& v : verticesVector[i])
		{
			...
			for (int j = 0; j < verticesVector.size(); j++)
			{
				...
				{
					Intersection* intersection = intersect_ray_triangle(r,
					    verticesVector[j][indicesVector[j][k]].position,
					    verticesVector[j][indicesVector[j][k + 1]]
					        .position,
					    verticesVector[j][indicesVector[j][k + 2]]
					        .position);
